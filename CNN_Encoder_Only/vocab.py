...
class VocabEntry(object):
    ...
    def __init__(self, word2id=None):
        ...
        if word2id:
            ...
            self.word2id['<s>'] = 2 # Start Token
            self.word2id['</s>'] = 3    # End Token
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        sent_ints = self.words2charindices(sents)
        sents_padded = pad_sents_char(sent_ints,self.char2id['<pad>'])
        #sents_padded shape: (batch_size, max_sentence_length, max_word_length)
        sents_var = torch.tensor(sents_padded, dtype=torch.long, device=device) #this is probably dtype = int
        sents_var = sents_var.permute(1, 0, 2).contiguous()
        #sents_var shape should be: (max_sentence_length, batch_size, max_word_length)
        return sents_var
        ### END YOUR CODE
...
if __name__ == '__main__':
    #this is entirely used to build the vocabulary from a gensim word2vec model.
    args = docopt(__doc__)
