...
def predict(vocab2):
	...
	with open("biobert/model.th", 'rb') as f:
		model2.load_state_dict(torch.load(f))
	
	predictor2 = SentenceClassifierPredictor(model2, dataset_reader=reader)
	with open('biobert/predictions.txt', 'w+') as f:
		top_10_words_list = []
	...
	np.save('biobert/top_10_words_list.npy', np.array(top_10_words_list))
...
if __name__ == '__main__':
	vocab = main()
	predict(vocab)
	eval_predictions("biobert/predictions.txt", DATA_ROOT + "/" + "test_all.txt")
